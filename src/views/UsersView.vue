<template>
  <div class="users-view">
    ...
    <div class="filter-section" v-if="authStore.userInfo">
      <label for="orgFilter">조직 선택:</label>
...
const canUseAdminApi = computed(() => authStore.isSystemAdmin());
...
async function fetchUsers() {
  ...
  try {
    let data;
    if (canUseAdminApi.value) {
      data = await apiService.adminListUsers(selectedOrganizationId.value);
    } else {
      data = await apiService.listUsers(selectedOrganizationId.value);
    }
...
async function submitForm() {
  ...
  try {
    if (isEditing.value && currentUserId.value) {
      ...
      if (canUseAdminApi.value) {
        await apiService.adminCreateUser(selectedOrganizationId.value, createData);
      } else {
...
async function confirmDelete(user: any) {
  ...
  try {
    if (canUseAdminApi.value) {
      await apiService.adminDeleteUser(user.id, selectedOrganizationId.value);
    } else {
